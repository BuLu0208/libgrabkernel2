...
static NSString *getFirmwareURLFromAll(NSString *osStr, NSString *build, NSString *modelIdentifier, bool *isOTA) {
    ...
    if (!compressed) {
        ERRLOG("Failed to fetch firmware list: %s\n", error.localizedDescription.UTF8String);
        return nil;
    }
    ...
    for (NSDictionary *device in devices) {
        ...
        if (![identifier isEqualToString:modelIdentifier]) continue;
        
        NSArray *firmwares = device[@"firmwares"];
        if (![firmwares isKindOfClass:[NSArray class]]) continue;
        
        for (NSDictionary *firmware in firmwares) {
            if (![firmware isKindOfClass:[NSDictionary class]]) continue;
            
            NSString *firmwareBuild = firmware[@"buildid"];
            if (![firmwareBuild isKindOfClass:[NSString class]]) continue;
            
            if (![firmwareBuild isEqualToString:build]) continue;
            
            NSArray *sources = firmware[@"sources"];
            if (![sources isKindOfClass:[NSArray class]]) continue;
            
            return bestLinkFromSources(sources, modelIdentifier, isOTA);
        }
    }
